...
class ApiClient {
  ...
  async saveInstagramConfig(data: Record<string, unknown>) {
    return this.request('/instagram/config', { method: 'PUT', body: JSON.stringify(data) })
  }

  async disconnectInstagram() {
    return this.request('/instagram/config', { method: 'DELETE' })
  }

  async sendInstagramMessage(data: Record<string, unknown>) {
    return this.request('/instagram/send', { method: 'POST', body: JSON.stringify(data) })
  }
  ...
  async getInstagramConversation(id: string) {
    return this.request(`/instagram/conversations/${id}`)
  }

  async updateInstagramConversation(id: string, data: Record<string, unknown>) {
    return this.request(`/instagram/conversations/${id}`, {
      method: 'PUT',
