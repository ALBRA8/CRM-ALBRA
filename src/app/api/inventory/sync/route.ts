...
import { refreshGoogleToken } from '../../google/config/route';

// POST /api/inventory/sync - Trigger manual sync from Google Sheet → Service model
  ...
  try {
    ...
    if (!inventoryConfig) {
      return NextResponse.json(
        { error: 'Configuración de inventario no encontrada' },
        { status: 400 }
      );
    }
    ...
    if (!googleConfig || !googleConfig.accessToken) {
      return NextResponse.json(
        { error: 'Google no está conectado. Conecta tu cuenta primero.' },
        { status: 400 }
      );
    }
    ...
    if (googleConfig.tokenExpiry && new Date(googleConfig.tokenExpiry) < new Date()) {
      if (!googleConfig.refreshToken) {
        return NextResponse.json({ error: 'Token expirado. Reconecta Google.' }, { status: 401 });
      }
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: 'Error al leer Google Sheet. Verifica el ID y permisos.' },
        { status: 500 }
      );
    }
    ...
    if (rows.length === 0) {
      return NextResponse.json({
        success: true,
        summary: { created: 0, updated: 0, skipped: 0, errors: 0, totalRows: 0 },
      });
    }
    ...
    for (const row of dataRows) {
      try {
        ...
        const existing = await db.service.findFirst({
          where: { userId: authResult.userId, name },
        });
